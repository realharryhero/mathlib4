...
protected theorem image {q : ℚ_[p]} : q ≠ 0 → ∃ n : ℤ, ‖q‖ = ↑((p : ℚ) ^ (-n)) :=
  Padic.induction_on q fun f hf ↦
    ...
    ⟨n, by rw [← hn, ← padicNormE.is_norm, padicNormE_mk]⟩
#align padic_norm_e.image padicNormE.image
...
theorem norm_eq_pow_val {x : ℚ_[p]} : x ≠ 0 → ‖x‖ = (p : ℝ) ^ (-x.valuation) := by
  ...
  rw [← padicNormE.is_norm, padicNormE.padicNormE_mk, valuation_mk, PadicSeq.norm_eq_pow_val]
